...
contract Exchange is ERC20 {
	...
	function addLiquidity(uint _amount) public payable returns (uint) {
		...
		/*
			If the reserve is emtpy, intake any user supplied value for
		...
		*/
		if(cryptoDevTokenReserve == 0) {
			...
			// `liquidity` tokens that need to be minted to the user on `addLiquidity` call should always be proortional
			...
			/*
				If the reserve is not empty, intake any ser supplied value for
			...
			*/
			...
			// transfer only (cryptoDevTokenAmount user can add) amount of `Crypto Dev tokens` from user's account to the contract
	...
	/**
	 * @dev Resturns the amount Eth/Crypto Dev tokens that would be returned to the user in the swap
	 */
	function removeLiquidity(uint _amount) public returns (uint, uint) {
		...
		// Then by some maths -> (Eth sent back to the user) = (current Eth reserve * amount of LP tokens that user wnat to withdraw)/(total supply of LP tokens)
		uint ethAmount = (ethReserve * _amount)/_totalSupply;

		// The amount of Crypto Dev token that would be sent back to the user is based on a ratio
		// Ratio is -> (Crypto Dev sent back)/(current Crypto Dev token reserve) = (amount of LP tokens to withdraw)/(total supply of LP tokens)
		// Then by some maths -> (Crypto Dev sent back) = (current CryptoDev token reserve * amount of LP to withdraw)/(total supply of LP tokens)
	...
	function getAmountOfTokens(
		...
		// Input amount with fee = (intput amount - (1*(input amount)/100)) = ((input amount)*99)/100
		...
		// So by putting the values in hte fomulae you can get the munerator and denominator
